...
The key feature of this package include 
...
- an adaptive adjacency construction framework with multiple options for building similarity graphs including locally-adaptive scaling (Zelnik-Manor and Peroni, 2004), 
...
3. Optionally, examine results results with 2D visualization function \(\textt{plot.fuzzy()}\) or compare to true class labels using \(\texttt{clustering.accuracy}\).
...
data <- gen.fuzzy(n = 300, 
                  dataset = "spirals", 
                  noise = 0.15)         # data generation


plot.fuzzy(data, plotFuzzy = TRUE, 
           colorCluster = TRUE)         # plot true data generating process
```
...
```{r}
acc <- clustering.accuracy(data$y, res$cluster)
...
plot.fuzzy(resDF, plotFuzzy = TRUE, colorCluster = TRUE)
```
...
* \(\textt{isModWeighted}\): apply similarity upweighting/downweighting
