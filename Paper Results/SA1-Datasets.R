...
# # Random Clusters with Varying Sizes and Overlap
# # Fixed covariance, fixed number of clusters, varying observations per cluster
# 
# library(mvtnorm)
# 
# generate_random_clusters_fuzzy <- function(nPts = 500, 
#                                            k = 20, 
#                                            seed = NULL, 
#                                            space_range = c(0, 30),
#                                            size_range = c(20, 80),
...
#   if (!is.null(seed)) set.seed(seed)
#   
#   # Default covariance matrix if not provided
#   if (is.null(cov_matrix)) {
#     cov_matrix <- diag(2)  # Identity matrix (circular clusters)
#   }
#   
#   # Generate random cluster centres
#   centres <- matrix(runif(k * 2, min = space_range[1], max = space_range[2]), 
#                     ncol = 2)
#   
#   # Generate random sizes for each cluster
#   cluster_sizes <- sample(size_range[1]:size_range[2], k, replace = TRUE)
#   
#   # Ensure we get approximately nPts total
#   scale_factor <- nPts / sum(cluster_sizes)
#   cluster_sizes <- round(cluster_sizes * scale_factor)
#   
#   # Adjust to get exactly nPts
#   diff <- nPts - sum(cluster_sizes)
#   if (diff > 0) {
#     # Add points to random clusters
#     add_to <- sample(1:k, diff, replace = TRUE)
#     for (i in add_to) cluster_sizes[i] <- cluster_sizes[i] + 1
#   } else if (diff < 0) {
#     # Remove points from largest clusters
#     while (diff < 0) {
#       largest <- which.max(cluster_sizes)
#       cluster_sizes[largest] <- cluster_sizes[largest] - 1
#       diff <- diff + 1
#     }
#   }
#   
...
#   datlist <- list()
#   for (i in 1:k) {
#     datlist[[i]] <- mvtnorm::rmvnorm(cluster_sizes[i], 
#                                      mean = centres[i,], 
#                                      sigma = cov_matrix)
#   }
#   
...
#   
#   # True cluster labels
#   yTrue <- unlist(mapply(rep, 1:k, cluster_sizes))
#   
#   # Calculate fuzzy memberships based on Gaussian mixture model
#   nTotal <- nrow(X)
#   uFuzzy <- matrix(0, nTotal, k)
#   
#   # Calculate prior probabilities based on cluster sizes
#   PIvec <- cluster_sizes / nTotal
#   
#   # Calculate density for each point under each cluster
#   densities <- matrix(0, nTotal, k)
#   for (j in 1:k) {
#     densities[, j] <- mvtnorm::dmvnorm(X, mean = centres[j,], sigma = cov_matrix)
#   }
#   
#   # Calculate weighted densities (likelihood × prior)
#   weighted_densities <- sweep(densities, 2, PIvec, "*")
#   
#   # Calculate posterior probabilities (fuzzy memberships)
#   densTotalensity <- rowSums(weighted_densities)
#   
#   # Handle numerical issues
#   zero_density <- densTotalensity == 0
#   if (any(zero_density)) {
#     # For points with zero density, assign to nearest cluster
#     for (i in which(zero_density)) {
#       distances <- sqrt(rowSums((centres - matrix(X[i,], nrow = k, ncol = 2, byrow = TRUE))^2))
#       nearest <- which.min(distances)
#       uFuzzy[i, nearest] <- 1
#     }
#   }
#   
#   # Normal case: normalize to get probabilities
#   uFuzzy[!zero_density, ] <- weighted_densities[!zero_density, ] / densTotalensity[!zero_density]
#   
#   # Return results
#   return(list(
#     X = X, 
...
# 
# 
# # Function to generate with different covariance structures
# generate_random_clusters_fuzzy_elliptical <- function(nPts = 500, 
#                                                       k = 20, 
#                                                       seed = NULL,
#                                                       space_range = c(0, 30),
#                                                       size_range = c(30, 80),
#                                                       cov_type = "spherical") {
#   
#   # Define covariance matrix based on type
#   cov_matrix <- switch(cov_type,
#                        "spherical" = diag(2),
#                        "diagonal" = diag(c(2, 0.5)),  # Elliptical, axis-aligned
#                        "rotated" = {
#                          # Rotated elliptical
#                          rotation <- matrix(c(cos(pi/4), -sin(pi/4), sin(pi/4), cos(pi/4)), 2, 2)
#                          rotation %*% diag(c(2, 0.5)) %*% t(rotation)
#                        },
#                        "correlated" = matrix(c(1, 0.7, 0.7, 1), 2, 2)  # Positive correlation
#   )
#   
#   return(generate_random_clusters_fuzzy(
#     nPts = nPts,
#     k = k,
#     seed = seed,
